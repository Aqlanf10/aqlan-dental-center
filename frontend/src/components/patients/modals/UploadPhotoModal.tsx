...
export default function UploadPhotoModal({ isOpen, onClose, patientId, onSuccess }: UploadPhotoModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fileInput?.files?.length) {
        ...
        setError('يرجى اختيار ملف الصورة');
        setSaving(false);
        return;
      }
      ...
      await fetch(`${apiUrl}/patients/${patientId}/clinical-photos`, {
        method: 'POST',
  ...
  const categories = [
    { value: 0, label: 'فموية' },
    { value: 1, label: 'تجميلية' },
    { value: 2, label: 'تقويم' },
    { value: 3, label: 'جراحية' },
    { value: 4, label: 'أخرى' },
  ];
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-[#1a3a5c] font-[Tajawal]">رفع صورة سريرية</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input id="photo-upload-file" type="file" accept="image/*" className="w-full text-sm" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700">الفئة</label>
            <select value={form.category} onChange={e => setForm({ ...form, category: Number(e.target.value) })} className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm focus:border-[#3d7ab5] focus:outline-none focus:ring-1 focus:ring-[#3d7ab5] font-[Tajawal]">
